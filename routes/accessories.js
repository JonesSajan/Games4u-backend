...
router.post('/addnewaccessories',upload.any(), function(req, res, next) {
    pool.query("insert into accessories(categoryid,subcategoryid,accessoryname,description,price,picture,stock,rented,rentamt,offer) values(?,?,?,?,?,?,?,?,?,?)",
    [req.body.categoryid,
...
router.post('/findaccessory',function(req,res){

    pool.query("select * from accessories where categoryid=?",[req.body.categoryId], function(error,result){
        if(error)
        {
            res.status(500).json([])
        }
...
router.get('/offeraccessoriec',function(req,res){
...
router.post('/editicon',upload.single('picture'), function(req, res, next) {
    ...
    pool.query("update accessories set picture=? where accesseoryid=?",
    [   req.file.originalname,
        req.body.accesseoryid]
    ,function(error,result){
        if(error){
            res.status(500).json({result:false})
        }
...
router.post('/editaccessories', function(req, res, next){
    pool.query("update accessories set categoryid=?, subcategoryid=?, accessoryname=?, description=?, price=?, rentamt=?, offer=?, stock=?, rented=? where accessoryid=?", [req.body.categoryid, req.body.subcategoryid, req.body.accessoryname, req.body.description, req.body.price, req.body.rentamt, req.body.offer, req.body.stock, req.body.rented, req.body.accessoryid], function(error, result) {
...
router.post('/deleteaccessories', function(req, res, next) {
    ...
    pool.query("delete from accessories where  accesseoryid=?",
    [req.body.accesseoryid]
    ,function(error,result){
        if(error)
        {
